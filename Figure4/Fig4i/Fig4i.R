signture_geome <- function(RNA_full_exp,cli,v_title,genelist,list_com) {
  signture_geome_data <- function(RNA_full_exp,cli,genelist,v_title) {
    ...
    geome_score <- function(data,v_title) {
      a2 <- data.frame(geome=geometricmeanCol(data)) 
      colnames(a2)[which(colnames(a2)=="geome")]  <- v_title
      a2
    }
    a <- signture_profile(RNA_full_exp,cli,genelist) %>% geome_score(.,v_title) %>% mutate(RNA_Tumor_Sample_Barcode=rownames(.))
    a2 <- a[cli$RNA_Tumor_Sample_Barcode,] %>% left_join(cli,by="RNA_Tumor_Sample_Barcode") 
    a2
  }
  signture_geome_data_select <- function(w.T1,j) {
    col <- c("Response","RNA_Tumor_Sample_Barcode","Type")
    w.T1.1<- w.T1 %>% dplyr::select(col,j) 
    colnames(w.T1.1) <- c(col,"cell_type")
    w.T1.1
  }
  signture_geome_plot <- function(data,j,list_com) {
  ...
  data <- signture_geome_data(RNA_full_exp,cli,genelist,v_title) %>% 
    signture_geome_data_select(.,v_title) %>% 
    signture_geome_plot(.,v_title,list_com)
  data
}
...
dir <- "/Users/yuanjingnan/Desktop/daily/20221212/ESCC材料汇总.20221212/figure/Figure/Figure4/Fig4i/"
...
genelist <- c("GZMA","PRF1")
v_title <- "Immune cytolytic activity"
P <-signture_geome(RNA_ESCC_full_exp,list_ESCC,v_title,genelist,list_com)
...
genelist <- c("HLA-DPA1","HLA-DPB1","HLA-DQA1","HLA-DQA2","HLA-DQB1","HLA-DQB2",
              "HLA-DRA","HLA-DRB1","HLA-DRB2","HLA-DRB3","HLA-DRB4","HLA-DRB5")
v_title <- "HLA-II"
P1 <- signture_zscore_mean(RNA_ESCC_full_exp,list_ESCC,v_title,genelist,list_com)
pdf(paste(dir,"mean.MHCII.pdf",sep = ""),width=6.5, height=5)
print(P1)
dev.off()


genelist <- c("CLEC9A","FLT3","XCR1")
v_title <- "DC"
P1 <- signture_zscore_mean(RNA_ESCC_full_exp,list_ESCC,v_title,genelist,list_com)
